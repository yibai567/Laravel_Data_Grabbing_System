...
class TaskCrawlOverTimeAlarm extends Command
{
    ...
    public function handle()
    {
        ...
        foreach($tasks as $task) {
            ...
            if (time()- strtotime($lastJobAt) > $alarmTime * 60) {
                $data = [
                    'type' => AlarmResult::TYPE_WEWORK,
                    'content' => '任务已超过' . $alarmTime . '分钟未执行成功,任务id: ' . $task['id'] . ',脚本id: ' . $task['script_id'] . ',脚本名称: ' . $task['name'] . '，脚本类型：'. Task::DATA_TYPE_ARRAY[$task['data_type']] . '，最后执行时间: ' . $lastJobAt,

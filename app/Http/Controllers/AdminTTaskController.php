	...
	class AdminTTaskController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
            ...
            $this->col[] = ["label"=>"语言类型","name"=>"languages_type","callback"=>function ($row) {
                if( $row->languages_type == Task::LANGUAGES_TYPE_CASPERJS) {
                    return 'casperJs';
                } else if ($row->languages_type == Task::LANGUAGES_TYPE_HTML) {
                    return 'html';
                } else if ($row->languages_type == Task::LANGUAGES_TYPE_API) {
                    return 'api';
                }
            }];
            ...
            $this->col[] = ["label"=>"状态","name"=>"status","callback"=>function ($row) {
                ...
                } else if ($row->status == Task::STATUS_STOP) {
                    return "<a class='btn btn-xs btn-danger'><i></i>已停止</a>";
                }
            }];
			...
			$this->form[] = ['label'=>'Languages Type','name'=>'languages_type','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];

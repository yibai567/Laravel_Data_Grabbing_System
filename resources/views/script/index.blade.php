...
@section('content')
        ...
        <ul id="foo" class="block__list block__list_words" style="overflow-y: auto; overflow-x: hidden;">
            ...
            <li>
                ...
                <p>模块名:{{$value->name}}</p>
                <!-- <span>模块描述:{{$value->description}}</span> -->
                <!-- <span>代码结构:{{$value->structure}}</span> -->
                <!-- <span>参数规则:{{$value->parameters}}</span> -->
                <br>
                <!-- @for($i=0;$i<count(json_decode($value->parameters));$i++) -->
                <!-- <span> -->
                    <!-- <textarea name="script_model_params[{{$value->id}}][]" maxlength=5000 rows="3" cols="50"></textarea> -->
                <!-- </span> -->
                <!-- </span> -->
                <!-- @endfor -->
                ...
                @foreach(json_decode($value->parameters, true) as $parametersValue)
                    ...
                    @if ($parametersValue[type] == 'string')
                        <span><input type="type" {{$requires}} name="script_model_params[{{$value->id}}][]"></span>
                        <br>
